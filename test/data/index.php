...
class Link {
    ...
    public function checkDataIfExist(string $data){
        $tmp = $this->first;
        if(is_null($tmp)) {
            return false;
        }
...
function getAccount(){
    ...
    $link = new Link();
    while ($link->checkDataIfExist($new_Account)){
        $new_Account = mt_rand(10000000,99999999);
    }
    echo $new_Account;
}
