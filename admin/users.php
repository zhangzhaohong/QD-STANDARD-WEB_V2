...
elseif($my=='add_submit')
{
    ...
    if($account==NULL or $password==NULL or $private_name==NULL or $user_birthday==NULL or $user_email==NULL or $user_level==NULL or $log_level==NULL or $user_available_date==NULL){
        ...
        if($check_account){
            ...
            if(!$DB->query("insert into `users` (`account`,`password`,`private_name`,`user_birthday`,`user_email`,`user_key`,`user_level`,`log_level`,`user_available_date`) values ('".$account."','".$password."','".$private_name."','".$user_birthday."','".$user_email."','".$user_key."','".$user_level."','".$log_level."','".$user_available_date."')"))
                echo '<script>FailedSettingMessage();</script>';
...
elseif($my=='edit_submit')
{
    ...
    if($password==NULL or $private_name==NULL or $user_birthday==NULL or $user_email==NULL or $user_level==NULL or $log_level==NULL or $user_available_date==NULL){
        ...
        if($DB->query("update users set password='$password',private_name='$private_name',user_birthday='$user_birthday',user_email='$user_email',user_level='$user_level',log_level='$log_level',user_available_date='$user_available_date' where jobid='{$jobid}'"))
            echo '<script>SuccessSettingMessage();</script>';
        else
            echo '<script>FailedSettingMessage();</script>';
    }
