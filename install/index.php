...
function checkAll(){
    ...
    try{
        ...
        $redis->set("TESTAll", "TESTAll");
        $redis->expire("TESTAll", 600);
        $test = $redis->get("TESTAll");
    ...
    if (function_exists("curl_exec") && function_exists("file_get_contents") && $_SESSION['checksession']==1 && $redis_status){
        echo "<span style='float:right'><a class='btn btn-primary' href='index.php?do=2' align='right'>下一步>></a></span>";
    }else{
        echo "<span style='float:right'><a class='btn btn-primary' href='index.php?do=1' align='right'>重试</a></span>";
    }
}
...
function checkRedis($m = false) {
    try{
        require '../includes/predis/autoload.php';
        //连接本地的 Redis 服务
        $redis = new Predis\Client();
        $redis->set("TEST", "TEST");
        $redis->expire("TEST", 600);
        $test = $redis->get("TEST");
        if ($redis->isConnected())
            $redis->disconnect();
        if ($test == null || $test == "")
...
function clearpack() {
    $array=glob('../data_release_*');
    foreach($array as $dir){
        unlink($dir);
    }
    $array=glob('../data_update_*');
    foreach($array as $dir){
        unlink($dir);
    }
}
...
<div class="container" style="padding-top:60px;">
    <div class="col-xs-12 col-sm-8 col-lg-6 center-block" style="float: none;">
        <?php if($do=='0'){
            ...
            <div class="panel panel-primary">
                <div class="panel-heading">
                    <h3 class="panel-title" align="center">DC自助授权系统管理中心</h3>
                </div>
        ...
        <?php }elseif($do=='1'){?>
            <link rel="stylesheet" href="../assets/css/install_common.css">
            <div class="container_info">
                <div class="panel panel-primary" style="width: 100%">
                    <div class="panel-heading" contenteditable="false">环境检查</div>
                    <div class="panel-body" contenteditable="false" style="padding: 0px;">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered scrollbar-hover" id="tableDataGrid">
                                ...
                                <tr>
                                    <td>Redis支持</td>
                                    <td>必须</td>
                                    <td><?php echo checkRedis(true); ?></td>
                                    <td>缓存数据</td>
                                </tr>
                                </tbody>
                            </table>
                        ...
                        <p style="margin: 10px">
                            <span><a class="btn btn-primary" href="index.php?do=0"><<上一步</a></span>
                            <?php
                            checkAll();
                            ?>
                        </p>
...
HTML;
		...
		<input type="text" class="form-control" name="db_qz" value="">
                        ...
                        if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
                            ...
                            if(!$con=DB::connect($dbconfig['host'],$dbconfig['user'],$dbconfig['pwd'],$dbconfig['dbname'],$dbconfig['port'])){
                                ...
                                if(DB::query("select * from ".$dbconfig['dbqz']."_config where 1")==FALSE)
                                    ...
                                    echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过该系统</div>
            ...
            <div class="panel panel-primary">
                ...
                <div class="panel-body">
                    ...
                    if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
                        ...
                        DB::query("INSERT INTO `config` VALUES ('build', '".$date."')");
                        DB::query("INSERT INTO `config` VALUES ('cronkey', '".rand(100000,999999)."')");
                        DB::query("INSERT INTO `config` VALUES ('syskey', '".random(32)."')");
                    }
                    if($e==0) {
